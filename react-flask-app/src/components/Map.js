...
const Direction = () => {
    ...
    mapRef.current.on("load", () => {
      ...
      if (routeGeometry) {
        ...
        mapRef.current.fitBounds(
          routeGeometry.coordinates.reduce(
  ...
  const calculateRoute = async () => {
    if (origin && destination) {
      try {
        const originResponse = await geocodingClient
          .forwardGeocode({
            query: origin,
            limit: 1,
          })
        ...
        const destinationResponse = await geocodingClient
          .forwardGeocode({
            query: destination,
            limit: 1,
          })
        ...
        const destinationCoordinates =
          destinationResponse.body.features[0].center;
